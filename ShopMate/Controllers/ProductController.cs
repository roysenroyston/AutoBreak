...
namespace ShopMate.Controllers
{
	public class ProductController : BaseController
	{
		...
		public ActionResult Create()
		{
			...
			//else
			{
				var products = db.Products.Where(n => n.WarehouseId == myProductWarehouse && n.ProductType == "CASE").ToList();
				var items = products.Select(p => new SelectListItem
				{
					Value = p.Id.ToString(),
					Text = p.Name + (p.ProductCaseId == null || p.ProductCaseId.Value == 0 ? " - P" : "")
				});
				ViewBag.ProductCaseId = new SelectList(items, "Value", "Text");
		...
		public ActionResult Create(Product ObjProduct, HttpPostedFileBase ProductImage, string HideImage1)
		{
			...
			using (var transaction = db.Database.BeginTransaction())
			{
				try
				{
					var productExist = db.Products.Where(i => i.Name == ObjProduct.Name && i.WarehouseId == ObjProduct.WarehouseId).FirstOrDefault();
					var productWarehouse = db.WarehouseStocks.Where(i => i.Product_ProductId.Name == ObjProduct.Name && i.WarehouseId == warehouse).Count();
		...
		public ActionResult Edit(int? id)
		{
			...
			if (myProductWarehouse == 1)
			{
				...
				{
					Value = p.Id.ToString(),
					Text = p.Name + (p.ProductCaseId.Value == 0 ? " - P" : "")
				});
				ViewBag.ProductCaseId = new SelectList(items, "Value", "Text", ObjProduct.ProductCaseId);
				ViewBag.WareHouse = new SelectList(db.Warehouses, "Id", "Name");
			}
			else
			{
